...
public://metodos (acciones)
    ...
    void eliminarp (int posicion_r){//recorre el vector desde que la posicion de eliminado sea igual a la posicon en el vector, ese lugar lo ocupa el de la derecha
        for (int i=posicion_r; i<size; i++){
            v[i]=v[i+1];
        }
        size--;//actualizamos el tam,ano del vector
    }
    
    //elimina todos los elementos
    void eliminar_a(int elementos){
        for (int i=0; i<size; i++){
            if (v[i]==elementos){//==para que sea exactamente el mismo
                eliminarp(i);//para usar la funcion
            }
...
int main() {
    ...
    int elementos;
    cout << "\nIngrese el/los elementos que desea eliminar: ";
    cin >> elementos;
    v.eliminar_a(elementos);
